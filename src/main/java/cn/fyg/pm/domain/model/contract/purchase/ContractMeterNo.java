package cn.fyg.pm.domain.model.contract.purchase;

import org.springframework.stereotype.Component;

import cn.fyg.pm.domain.model.contract.general.ContractType;
import cn.fyg.pm.domain.model.nogenerator.generator.Pattern;
import cn.fyg.pm.domain.model.nogenerator.generator.impl.AbstractPattern;
import cn.fyg.pm.domain.model.nogenerator.generator.impl.AbstractPatternFactory;
import cn.fyg.pm.domain.model.nogenerator.norecord.NoKey;

@Component("contractMeterNo")
public class ContractMeterNo extends AbstractPatternFactory<ContractMeter> {
	
	private class PatternImp extends AbstractPattern<ContractMeter>{

		@Override
		public String getNo() {
			return this.t.getNo();
		}

		@Override
		public void setNo(String no) {
			this.t.setNo(no);
		}

		@Override
		public Long initLimmit() {
			return Long.valueOf(9999);
		}

		@Override
		public void initNoKey(NoKey noKey, ContractMeter contractMeter) {
			noKey.setSys("D");
			noKey.setFlag("HT");
			String projectNo=contractMeter.getProject().getNo();
			String noParts=projectNo.substring(3);
			noKey.setPref(noParts+ContractType.meter.getCode());
		}
	}

	@Override
	public Pattern<ContractMeter> doCreate(ContractMeter t) {
		return new PatternImp().init(t);
	}

}
